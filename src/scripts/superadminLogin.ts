...
function renderSuperadminLoginForm() {
  ...
  const superadminLoginBtn: HTMLButtonElement =
...
export async function superadminLoginCheck(loginData: object) {
  ...
  fetch('https://planning-poker-api-production.up.railway.app/superadmin', {
    method: 'POST',
    headers: {
...
export function renderSessionHistory() {
  const sessionHistoryHeading: HTMLHeadingElement =
    document.createElement('h1');
  const sessionHistorySubHeading: HTMLHeadingElement =
  ...
  sessionHistorySubHeading.innerHTML =
    'Klicka på ett namn i listan för att se den sparade sessionen';
  ...
  fetch('https://planning-poker-api-production.up.railway.app/sessions')
    .then((res) => {
      return res.json();
    })
    .then((data) => {
      ...
      dataFromDatabase.forEach((session: { adminName: any }) => {
        ...
        sessionHistoryLi.addEventListener('click', () =>
...
function renderSessionInfo(session: any) {
  ...
  const sessionHistoryAdminAndUsers: HTMLParagraphElement =
  ...
  session.topics.forEach((topic: { title: string; averageScore: number }) => {
    const topicTitleAndScore: HTMLParagraphElement =
      document.createElement('p');
...
export function superadminLogout() {
  const superadminLogoutBtn: HTMLButtonElement =
    document.createElement('button');
